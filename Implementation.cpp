...
ostream& operator <<(ostream &out,const Customer &obj1) {
	...
	if(obj1.roomAlloted)
		out<<"Room alloted: "<<obj1.roomAlloted->getRoomNo()<<endl;
	else
		out<<"Room not alloted yet."<<endl;
	if(obj1.orderedFoods)
	{
		for(int i=0; i<(obj1.orderedFoods[0]->getFoodQuantity()); i++) {
			out<<"Food item "<<i+1<<" ordered: "<<obj1.orderedFoods[i]->getFoodOrdered()->getName()<<endl;
		}
	}
	else
	{
		out<<"No Food Ordered yet."<<endl;
	}
	return out;
}
...
void Customer::printinfo() {
	...
	if(roomAlloted)
		...
		cout<<"Room not alloted yet."<<endl;
	if(orderedFoods)
	{
